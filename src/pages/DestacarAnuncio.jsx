...
import { Zap, Star, Crown, ArrowLeft, Check, CreditCard, ChevronDown, ChevronUp, Info } from 'lucide-react';
...
const HIGHLIGHT_PLANS = [
  {
    ...
    features: [
      ...
      'Visibilidade por tipo de veículo',
      'Visibilidade por marca',
  ...
  },
  {
    ...
    features: [
      ...
      'Destaque nos resultados de busca',
      'Visibilidade por tipo, marca e região',
      'Validade de 7 dias',
...
function InstallmentInfo({ plan, installments }) {
  ...
  for (let n = 1; n <= installments.withFee; n++) {
    ...
    if (n > installments.free) {
      // compound fee approximation
      total = plan.price * Math.pow(1 + installments.feePercent / 100, n - installments.free);
    }
    ...
    <div className="mt-3">
      ...
      {open && (
        <div className="mt-2 bg-zinc-50 rounded-xl border border-zinc-200 divide-y divide-zinc-100 text-xs overflow-hidden">
          {options.map(opt => (
            <div key={opt.n} className={cn(
              "flex items-center justify-between px-3 py-2",
              !opt.hasFee && "text-green-700 font-medium"
            )}>
              ...
              <span className={opt.hasFee ? "text-zinc-400" : "text-green-600"}>
                {opt.hasFee
                  ? `Total: ${opt.total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} (+${installments.feePercent}% a.m.)`
                  : 'Sem juros'}
              </span>
            </div>
          ))}
...
function PlanCard({ plan, selected, onSelect }) {
  ...
  return (
    <div
      ...
      className={cn(
        "relative bg-white rounded-2xl border-2 overflow-hidden flex flex-col cursor-pointer transition-all duration-200",
        selected ? "border-red-500 ring-2 ring-red-200 shadow-lg" : plan.color + " hover:shadow-md",
        plan.badge === 'Mais vendido' && !selected && "ring-2 ring-amber-300 ring-offset-1"
      )}
    >
      {plan.badge && (
        <div className="absolute top-4 right-4">
          <span className={cn(
            ...
            plan.badge === 'Mais vendido' ? "bg-amber-500 text-white" : "bg-red-600 text-white"
          )}>
            {plan.badge}
          </span>
        </div>
      ...
      <div className={cn("px-6 pt-6 pb-5", plan.headerBg)}>
        ...
        {plan.installments && (
          <p className="text-xs text-zinc-400 mt-1">
            ou 2x de {(plan.price / 2).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} sem juros
          </p>
        )}
      </div>
      ...
      <div className="px-6 pb-6">
        <div className={cn(
          "w-full h-10 rounded-xl flex items-center justify-center gap-2 text-sm font-semibold transition-colors",
          selected
      ...
      <main className="max-w-5xl mx-auto px-4 py-12 space-y-10">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-zinc-500 max-w-lg mx-auto">
            Seu anúncio aparece em posição privilegiada nos resultados de busca e na página inicial, aumentando as chances de venda.
          </p>
        </div>
            ...
            {plan.installments && (
              <div>
                ...
                <select
                  ...
                  className="w-full border border-zinc-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-red-400 bg-white"
                >
              ...
              <div className="text-right">
                ...
                {installments > 1 && (
                  <div className="text-xs text-zinc-400">
                    {installments}x de {perMonth.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                  </div>
                )}
              </div>
            </div>

            <Button className="w-full bg-red-600 hover:bg-red-700 text-white rounded-xl h-12 font-bold gap-2">
              <CreditCard className="w-5 h-5" />
              Pagar e ativar destaque
            </Button>

            <p className="text-xs text-center text-zinc-400">
              🔒 Pagamento seguro. O destaque é ativado imediatamente após a confirmação.
            </p>
          </div>
        )}
      </main>
